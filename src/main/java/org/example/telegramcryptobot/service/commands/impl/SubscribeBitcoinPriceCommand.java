...
public class SubscribeBitcoinPriceCommand extends AbstractBotCommand {
    private final static String INCORRECT_USER_REQUEST = "The required price is not specified: ";

    private final int removingCommandFromMessage = BotCommandType.SUBSCRIBE_BITCOIN_PRICE.getCommand().length();
    ...
    public SendMessage getMessageResponse(Update update) {
        ...
        if(isNumeric(userMessage)) {
            ...
            String bitcoinPrice = getPriceBitcoinCommand.getMessageResponse(update).getText();
